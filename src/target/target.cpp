...
{
	target *t = static_cast<target*>(calloc(1, sizeof(*t)));
	...
	t->attach = reinterpret_cast<decltype(target::attach)>((void*)nop_function);
	t->detach = reinterpret_cast<decltype(target::detach)>((void*)nop_function);
	t->mem_read = reinterpret_cast<decltype(target::mem_read)>((void*)nop_function);
	t->mem_write = reinterpret_cast<decltype(target::mem_write)>((void*)nop_function);
	t->reg_read = reinterpret_cast<decltype(target::reg_read)>((void*)nop_function);
	t->reg_write = reinterpret_cast<decltype(target::reg_write)>((void*)nop_function);
	t->regs_read = reinterpret_cast<decltype(target::regs_read)>((void*)nop_function);
	t->regs_write = reinterpret_cast<decltype(target::regs_write)>((void*)nop_function);
	t->reset = reinterpret_cast<decltype(target::reset)>((void*)nop_function);
	t->halt_request = reinterpret_cast<decltype(target::halt_request)>((void*)nop_function);
	t->halt_poll = reinterpret_cast<decltype(target::halt_poll)>((void*)nop_function);
	t->halt_resume = reinterpret_cast<decltype(target::halt_resume)>((void*)nop_function);
	t->check_error = reinterpret_cast<decltype(target::check_error)>((void*)false_function);
...
void target_mem_map_free(target *t)
{
	while (t->ram) {
		...
		free(t->ram);
		t->ram = static_cast<target_ram*>(next);
	}
	while (t->flash) {
		...
		if (t->flash->buf)
			free(t->flash->buf);
		free(t->flash);
		t->flash = static_cast<target_flash*>(next);
	}
}
...
void target_list_free(void)
{
	...
	while(target_list) {
		...
		while (target_list->bw_list) {
			...
			target_list->bw_list = static_cast<breakwatch*>(next);
		}
		free(target_list);
		target_list = t;
	}
...
{
	struct target_command_s *tc = static_cast<target_command_s*>(malloc(sizeof(*tc)));
...
{
	struct target_ram *ram = static_cast<target_ram*>(malloc(sizeof(*ram)));
                       ...
                       target_addr dest, const void *src, size_t len)
{
	...
	while (len) {
		...
		dest += tmplen;
		src = reinterpret_cast<const void*>(reinterpret_cast<uintptr_t>(src) + tmplen);
		len -= tmplen;
	}
	return ret;
}
                                ...
                                target_addr dest, const void *src, size_t len)
{
	...
	while (len) {
		...
		memcpy(reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(f->buf) + offset), src, sectlen);
		dest += sectlen;
		src = reinterpret_cast<const void*>(reinterpret_cast<uintptr_t>(src) + sectlen);
		len -= sectlen;
	}
	return ret;
}
...
void target_regs_read(target *t, void *data)
{
	...
	for (size_t x = 0, i = 0; x < t->regs_size; ) {
		x += t->reg_read(t, i++, reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(data) + x), t->regs_size - x);
	}
}
void target_regs_write(target *t, const void *data)
{
	...
	for (size_t x = 0, i = 0; x < t->regs_size; ) {
		x += t->reg_write(t, i++, reinterpret_cast<const void*>(reinterpret_cast<uintptr_t>(data) + x), t->regs_size - x);
	}
}
                          ...
                          enum target_breakwatch type, target_addr addr, size_t len)
{
	...
	if (ret == 0) {
		...
		struct breakwatch *bwm = static_cast<breakwatch*>(malloc(sizeof bw));
